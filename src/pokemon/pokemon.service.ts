...
export class PokemonService {
  ...
  {
    return this.httpService.get(`${this.pokeApiUrl}/pokemon?limit=100`).pipe(
        ...
        const pokemon = {
          ...
          types: await Promise.all(
            response.data.types.map(async (t) => {
              const typeData = await lastValueFrom(
                this.httpService.get(t.type.url)
              );
              const names = typeData.data.names
                .filter(
                  (n) => n.language.name === 'es' || n.language.name === 'ja'
                )
                .map((n) => ({
